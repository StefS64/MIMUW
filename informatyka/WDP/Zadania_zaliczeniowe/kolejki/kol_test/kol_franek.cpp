...
void zwolnij_liste(lista &l) // funkcja zwalnia pamięć po głowie i ogonie listy (atrapach), ale nie zwalnia interesantów (to robi użytkownik)
{
    free(l.glowa);
    free(l.ogon);
}
...
vector<interesant *> fast_track(interesant *i1, interesant *i2)
{
    ...
    while (akt1 != NULL && akt2 != NULL && akt1 != i2 && akt2 != i2) // wychodzimy z pętli gdy dojdziemy jednym wskaźnikiem do końca listy albo spotkamy i2
    {
        wyn1.push_back(akt1);
        wyn2.push_back(akt2);
