...
double min_wartosc(wartosc w){
   if(w.czy_anty){
      ...
      if((bool)isinf(w.x)){//isinf niech będzie true lub false nie ma specyfikacji, że isinf(-HUGE_VAL) = -1
         return -HUGE_VAL;
      }
      return w.x;
   }
...
int main(){
   ...
   wartosc a = wartosc_od_do((-1.), 0.125);
a = podzielic(mjeden, a);  /* [-∞,-8]•[1,+∞] */
a = plus(mjeden, a);  /* [-∞,-9]*[0,+∞] */
a = plus(mjeden, a);  /* [-∞,-10]•[-1,+∞] */
wartosc b = wartosc_od_do((-1.), (1./3.));
b = podzielic(jeden, b);  /* [-∞,-1]•[3,+∞] */
b = plus(jeden, b);  /* [-∞,0]•[4,+∞] */
b = plus(jeden, b);  /* [-∞,1]•[5,+∞] */
wartosc c = razy(b, a); /* [-∞,+∞] */
printf("%d", in_wartosc(c,1.));
}
