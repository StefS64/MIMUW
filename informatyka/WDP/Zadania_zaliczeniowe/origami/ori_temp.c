...
int stron(zespo X, zespo A, zespo B){
...
void input(int n, kartka *T){
    for(int i = 1; i <= n; i++){
        scanf(" %c", &T[i].typ);
        if(T[i].typ == 'P'){
            scanf("%lf %lf", &T[i].X1.re, &T[i].X1.im);
            scanf("%lf %lf", &T[i].X2.re, &T[i].X2.im);
        }
        else if(T[i].typ == 'K'){
            scanf("%lf %lf %lf", &T[i].X1.re, &T[i].X1.im, &T[i].r);
        }
        else if (T[i].typ == 'Z'){
            scanf("%d %lf %lf", &T[i].k, &T[i].X1.re, &T[i].X1.im);
            scanf("%lf %lf", &T[i].X2.re, &T[i].X2.im);
        }
    }
}
...
void print_input(int n, kartka *T){
    for(int i = 1; i <= n; i++){
        printf(" %c ", T[i].typ);
        if(T[i].typ == 'P'){
            printf("%lf %lf ", T[i].X1.re, T[i].X1.im);
            printf("%lf %lf\n", T[i].X2.re, T[i].X2.im);
        }
        else if(T[i].typ == 'K'){
            printf("%lf %lf %lf\n", T[i].X1.re, T[i].X1.im, T[i].r);
        }
        else if (T[i].typ == 'Z'){
            printf("%d %lf %lf ", T[i].k, T[i].X1.re, T[i].X1.im);
            printf("%lf %lf\n", T[i].X2.re, T[i].X2.im);
        }
...
int ile_przebic(int num_k, zespo X,kartka *T){
    ...
    if(T[num_k].typ == 'P'){
        ...
        if(stron(X,T[num_k].X1, T[num_k].X2) == 1){
            return 0;
        }else{
            if(czy_w_okregu(X, X_odbit, EPSILON)){
